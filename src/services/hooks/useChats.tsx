...
export const getChatByAccount = async (accountId: string): Promise<Message[]> => {
    if (accountId) {
      const { data } = await api.get(`/clients/chatByAccount?account=${accountId}`);
  ...
  export function useChatByAccount(accountId: string) {
    return useQuery(['ChatByAccount', accountId], () => getChatByAccount(accountId));
  }
