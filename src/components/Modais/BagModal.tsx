  ...
  return (
    <Flex>
      <Box cursor="pointer" onClick={onOpen}>
        <Flex flexDir={'column'} justify={'center'} align='center' fill="white" bg="gray.800" borderRadius={50} boxShadow={'5px 2px 6px rgba(0,0,0,0.4)'} position="fixed" w={['4rem',  '4rem', '4rem','4rem']} h={['4rem', '4rem','4rem']} top={['70vh']} left={['80vw', '80vw', '85vw', '90vw']}>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent bg="gray.900">
          ...
          <ModalBody>
              ...
              {Object.keys(cart).map(key => {
                return (
                  <Flex key={key} w='100%' h='100%' mb='1rem'>
                    <Image w={['8rem']} h={['10rem']} src={cart[key].product.photos[0].url} />
                    <Flex ml='1rem'  flexDir={'column'} fontFamily={'Anek Devanagari'} align={'start'}>
                      <Text>Nome: {cart[key].product.name}</Text>
                      <Text>Preço Unitário: R$ {cart[key].product.price}</Text>
                      <Text>Subtotal: R$ {cart[key].quantity * cart[key].product.price}</Text>
                      <Text>Descrição:</Text>
                      <Text maxH="6rem" css={{
                      '&::-webkit-scrollbar': {
                        width: '4px',
                    ...
                    }} overflowY="auto" w={'15rem'}>{cart[key].product.description}</Text>
                    <Flex align={'center'} as='form' w='100%'>
                      <Button bg='gray.700' _hover={{ background: 'gray.900'  }} h='2rem' onClick={() => removeFromCart(cart[key].product.id)}>
                          <BsFillTrashFill  color='#fff' />
                      </Button>
                      <Flex mb={'1rem'} align={'center'} ml={'2.2rem'} flexDir={'column'}>
                    ...
                    <HStack justify={'center'} width={'100%'}>
                      <Button bg='gray.700' _hover={{ background: 'gray.900'  }} h='2rem' onClick={() => decreaseAmount(cart[key].product)}>
                        <AiOutlineMinus color='#fff' />
                      </Button>
                      <Button bg='gray.700' _hover={{ background: 'gray.900'  }} h='2rem' onClick={() => addToCart(cart[key].product)}>
            ...
            <Text>Total: {String(total.toFixed(2)).replace('.', ',')}</Text>
            <VStack as='form' onSubmit={handleSubmit(onSubmit)}>
            <RadioGroup defaultValue='2'>
              <HStack>
                <Radio value='pix' {...register('typeOfPayment')}>Pix</Radio>
                <Radio value='picpay' {...register('typeOfPayment')}>PicPay</Radio>
              </HStack>
            </RadioGroup>
            <FormControl>
            ...
            <VStack justify={'center'} align='center'>
              {!imagemIsLoading ? 
                ...
                <VStack>
                  ...
                  <Text>Pronto!</Text>
                  <Text>Agora é só efetuar o pagamento que assim que for finalizado você receberá uma mensagem</Text>
                </VStack>
