...
import { createPhotos, updateProduct, useProductById } from '../../services/hooks/useProducts';
...
const DetailsProductModal: ForwardRefRenderFunction<DetailsProductModalHandle, ProductProps> = ({ product }: ProductProps, ref) => {
        ...
        <ModalContent bg="gray.900">
          ...
          <ModalBody>
            <Flex flexDir={['column', 'column', 'row']} justify={'space-between'}>
              <Flex minW={'20rem'} pr={'0.5rem'} maxW={800} flexDir='column' as={'form'} onSubmit={handleSubmit(editSubmit)}>
                <Stack spacing="0.5">
                  <Input bg={'gray.800'} name="name" label="Nome" {...register('name')} placeholder={product.name} />
                  <Input bg={'gray.800'} name="price" label="Preço"  {...register('price')} />
                  <Textarea bg={'gray.800'} css={{
                  '&::-webkit-scrollbar': {
                    width: '4px',
                  },
                 ...
                 name="description" label="Descrição"  {...register('description')} />
                  <Input
                    bg={'gray.800'}
                    name="debitPoints"
              ...
              >
                ...
                <Flex  wrap={'wrap'} minW={['20rem', '20rem', '48rem']} h={'24.4rem'} 
                css={{
                  '&::-webkit-scrollbar': {
                    width: '4px',
                  },
                ...
                overflowY={'auto'}>
                  {isLoading ? (
                    ...
                    newProduct?.photos?.map(photo => (
                      <Flex p={'0.2rem'} key={photo.id} flexDir={'column'} align={'end'}>
                        <Flex _hover={{ color: '#ff0118' }} transition="color 200ms" mb={'0.5rem'}>
                          <AiFillCloseCircle size={30} cursor='pointer' onClick={() => openDeleteModal(photo.id)} />
                        </Flex>
                ...
                <Flex justify={['center', 'center', 'start','end']} p={'1rem'} as='form' onSubmit={handleSubmit(onUploadImageSubmit)}>
                  <HStack spacing={2}>
                    <InputFile ref={inputFileRef} />
                    <Button type="submit" disabled={!inputFileRef.current?.images.length && true} bg={'orange'} _hover={{ bg: 'orangeHover'}} size="lg">
                      Upload
                    </Button>
                  </HStack>
                </Flex>
