    ...
    return (
        <Flex bg='red' w='100%' h="100%">
            <VStack bg={'gray.800'} h="100%" w="25rem">
                {data?.map(chat => (
                    <HStack key={chat.id} align={'center'} bg={accountId === chat.id && 'gray.700'} _hover={{ background: 'gray.700' }} p="2rem" justifyContent={'space-between'} onClick={() => getData(chat.id)}
                    transition={['background 200ms']} cursor={'pointer'} justify='center' w='100%' h="3.5rem">
                        <Text>{chat.name}</Text>   
             ...
             <VStack bg={'gray.700'} w='100%'>
                 <VStack p='2rem' align={'flex-start'} w={'100%'} h='100%'>
                    {dataChatByAccount?.map((message) => (
                        <HStack key={message._id} bg='gray.800' borderRadius={'2rem'} pr='2rem'>
