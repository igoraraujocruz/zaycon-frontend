...
import { api } from '../../services/apiClient';
  ...
  const CheckBoxBase: ForwardRefRenderFunction<HTMLInputElement, CheckboxProps> = (
    ...
    return (
            <CheckboxGroup defaultValue={userPermissions?.map(permission => permission.name)}>
                {permissions.map(permission => 
                    <ChakraCheckbox key={permission.id} {...rest} value={permission.name} ref={ref}>{permission.name}</ChakraCheckbox>
                )}
            </CheckboxGroup>
        )
  };
